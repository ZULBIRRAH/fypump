<?php 
require "databaseFYP.php";
if($_SERVER['REQUEST_METHOD'] !='POST'){
    echo "<script> alert('Error: No data to save.'); location.replace('coordinatorPage.php') </script>";
    $conn->close();
    exit;
}
extract($_POST);
...
$save = $conn->query($sql);
if($save){
    echo "<script> alert('Schedule Successfully Saved.'); location.replace('coordinatorPage.php') </script>";
}else{
    echo "<pre>";
